class CategoriesController < ApplicationController
  ...
  RESOURCE_CLASSES = { 'category' => Category,
                       'subcategory' => SubCategory }.freeze
  SERIALIZERS = { 'category' => CategorySerializer,
                  'subcategory' => SubCategorySerializer }.freeze

  before_action :set_type, only: %i[index create update destroy]
  before_action :validate_type, only: %i[create update destroy]
  ...
  def index
    ...
    when 'all'
      ...
      pagy_subcat, subcategories = pagy(SubCategory.all,
                                        page_param: :subcategories_page)
      render json: {
    ...
    when *VALID_TYPES
      ...
      render json: {
        "#{@type}s": serialize(records, SERIALIZERS[@type]),
  ...
  def destroy
    if @resource.destroy
      render json: { message: "#{@type.capitalize} deleted successfully" },
             status: :ok
    else
      render json: { error: "Failed to delete #{@type}" },
             status: :unprocessable_entity
    end
  end
